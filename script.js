...
gsap.to(imageSeq, {
  frame: frameCount - 1,
  snap: "frame",  // 1st img or 2nd img show frame
  ease: "none",
  scrollTrigger: {
      scrub:1.8,   //reverse animation
      pin:true,
      trigger:"#home",
  },
  onUpdate: render
...
function scaleImage(img, ctx) {
  ...
  var centerShift_x = (canvas.width - img.width * ratio) / 2;  //img center
  var centerShift_y = (canvas.height - img.height * ratio) / 2;
  ctx.clearRect(0, 0, canvas.width, canvas.height);  //clear pervious img
  ctx.drawImage(img, 0, 0, img.width, img.height,    // recent img
...
gsap.to("#home canvas",{
  ...
  scrollTrigger:{
      scrub:.1,
      trigger:"#home",
      start:"bottom 100%",  //last white space animation
      // markers:true,
  },
})
...
let left = document.getElementsByClassName('ri-arrow-left-s-fill ')[0];
let right= document.getElementsByClassName('ri-arrow-right-s-fill')[0];
...
document.addEventListener("mousemove",function(details){
  document.querySelectorAll(".imge").forEach((elem) =>{
